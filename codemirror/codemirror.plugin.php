...
if (('none' == $rte) && $mode) {
	...
	<script type="text/javascript">
		...
		CodeMirror.defineMode("MODx-{$mode}", function(config, parserConfig) {
			var mustacheOverlay = {
				token: function(stream, state) {
					...
					/**
					 * Fixes HTML entity highlighting, see https://github.com/modxcms/evolution/pull/309 and 
					 * https://github.com/modxcms/evolution/issues/308
					 */
					if (stream.match(/&([^\s;]+;)?([^\s=]+=)?/)) {
						return "attribute";
					}
		...
		/**
		 * Fixes CodeMirror refresh issue with QuickManager+, see https://github.com/modxcms/evolution/issues/174
		 */
		(function() {
